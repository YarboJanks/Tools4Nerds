...
A PvP-focused Elder Scrolls Online addon that surfaces combat information you'd otherwise have to guess at â€” CC immunity windows, blocked attacks, critical hits, debuff count, Mara's Balm cooldown, queue pop-ups, and global cooldown tracking.
...
### Debuff Counter
Displays a live count of negative effects currently active on you. When the count reaches 6 or more, the counter flashes rapidly as a warning. The counter is draggable â€” click and drag it anywhere on screen. Font size and color are configurable independently of the other indicators.

### Mara's Balm Tracker
Automatically appears when 5 or more pieces of the Mara's Balm set are equipped (including backbar weapons) and hides when the set is removed. Displays **MARAS** in green when ready. When on cooldown, shows **MARAS 24s** (counting down) in red until it's ready again. Draggable â€” click and drag to reposition. Font size is configurable.
...
- **Animation style** â€” Ascending (bottom to top), Descending (top to bottom), or Radial
- **Icon desaturation** â€” greys out ability icons during the GCD
- **Ready animation** â€” flashes the slot when the GCD expires
- **Potion cooldown** â€” optionally extends the overlay to consumable slots (off by default)
...
1. Download and extract the `Tools4Nerds` folder into:
   ...
   ```
2. Ensure [LibAddonMenu-2.0](https://www.esoui.com/downloads/info7-LibAddonMenu.html) is also installed â€” it is required.
3. Launch ESO and enable **Tools 4 Nerds** in the AddOns menu.

## Usage

### Settings Panel
Open **Settings â†’ AddOns â†’ Tools 4 Nerds** to configure each feature:
...
Each feature section has a **Test** button to preview how that indicator looks without needing to be in combat. The Mara's Balm test runs a 5-second countdown so you can see the full red â†’ green transition, and remains visible even with the settings panel open.

### Keybinding
Assign a key to **Toggle Tools 4 Nerds** under **Settings â†’ Controls â†’ AddOns** to enable/disable the addon on the fly.

### Slash Commands
| Command | Description |
|---------|-------------|
| `/t4n debug` | Prints current state to chat â€” combat flag, target type, buff count, CC tracking status, and tick state. Useful for diagnosing why an indicator isn't showing. |
